...
- [x] Sharing a valid TikTok URL via the Shortcut produces a visible confirmation within 2 seconds, without waiting on extraction. Verified two ways: (1) code — `POST /api/recipes` (`src/api/routes/recipes.ts`) only calls `enqueueJob`, which inserts one `jobs` row and returns; it never awaits download/OCR/ASR/Claude extraction (that's the worker's job, started separately). (2) Live — minted a fresh device token via `POST /api/setup/device-token` against `https://recipecart-production.up.railway.app` and timed two real `POST /api/recipes` calls: 0.266s and 0.267s round-trip, both `201`s, well under the 2s target. `docs/ios-shortcut.md` §3.4–3.5 documents the Shortcut-side POST/response handling built against this same confirmed contract. (Test job rows created for this check were deleted afterward via `DELETE /api/recipes/:id`.)
- [ ] Sharing an invalid (non-TikTok) URL is rejected locally with no backend call. The Shortcut-side logic is real and correctly designed (`docs/ios-shortcut.md` §3.3: `Match Text` regex `tiktok\.com|vm\.tiktok\.com` run before any network action, with the POST unreachable from the "otherwise" branch) and the backend performs no URL-shape validation itself (confirmed live: `POST /api/recipes` with `{"sourceUrl":"not a url"}` returned `201` and created a job — by design, since format-checking is the Shortcut's job, not the API's). Marking as an accepted-evidence pass rather than a hard `[x]`, because the actual on-device regex branch can only be exercised in the real Shortcuts app on a physical iPhone, which this environment cannot drive — no shortcut file exists in the repo yet (`docs/ios-shortcut.md` §3 "no `.shortcut` file exists in this repo yet"), so this is a documented-and-reasoned pass, not a device-observed one.
- [x] Re-sharing an already-submitted URL surfaces "existing recipe" rather than creating a duplicate job. Verified at the job-creation layer, not just UI messaging: `enqueueJob` (`src/platform/jobs.ts`) derives a idempotency key from `(userId, resolved videoId, time bucket)` and returns the existing row with `created:false` when a match exists, without inserting a new `jobs` row — covered by `src/platform/jobs.test.ts`'s "short-link dedup" suite (dedupes even across two different `/t/<token>/` short links that resolve to the same video). Live-confirmed: resubmitting `https://www.tiktok.com/@testuser/video/730000000000000001` returned `HTTP 200`, `created:false`, and the identical `jobId` as the original submit — genuinely one job row, not a duplicate. Frontend surfaces this via `RecipesList.tsx`'s "View existing" / "Reprocess" UI when `created:false`.
- [x] Every ingredient shown in Review carries a confidence indicator and is editable. **Editable: yes** — `IngredientCard.tsx` supports quantity/unit edit (`submitEdit`/`handleQuantityBlur`/`handleUnitBlur`), remove (`handleRemove`), and `Review.tsx`'s `handleAddIngredient` supports manually adding one; all wired to real PATCH/POST endpoints. **Confidence indicator: fixed 2026-07-20.** The gap (Claude's per-field `confidence` band being computed then discarded before persistence) is closed end to end: migration `src/platform/migrations/003_ingredient_confidence.ts` adds a nullable `ingredients.confidence` text column (mirrored in the `IngredientsTable` type, `src/platform/database.ts`); `persist()` in `src/pipeline/extract/index.ts` now copies `ing.canonical_name_en.confidence` onto each inserted row; `IngredientDto` (`src/api/lib/dto.ts`) gained an optional `confidence?: "high" | "medium" | "low"` field, wired through both DB-row-to-DTO mapping functions (`src/api/routes/recipes.ts`'s and `src/api/services/recipe_edits.ts`'s `toIngredientDto`); and `IngredientCard.tsx` now renders `ConfidenceBadge` with the real band whenever `ingredient.confidence` is present, alongside (not replacing) the existing separate "amount unclear" badge. Covered by tests in `src/pipeline/extract/index.test.ts` (confidence survives persistence) and `src/api/services/recipe_edits.test.ts` (DTO carries confidence through on edit; manually-added ingredients correctly have no confidence).
- [x] No product is added to the Kroger cart without the item being visible and approved (implicitly via pre-selection or explicitly) in the Review screen first. Verified end-to-end through the actual write path, per the PRD's own "pre-selection counts as approval" intent: `persistMatch` (`src/matcher/index.ts` lines 1006-1051) only sets `is_approved:true` + a `selected_product_id` when `!match.requiresApproval` (a confident match auto-picks its own top candidate); a `requiresApproval` (ambiguous/material) match is inserted with `is_approved:false`, `selected_product_id:null`, forcing an explicit user pick via `MatchPicker.tsx` (`match.requiresApproval` forces the picker permanently expanded with nothing pre-checked). On the cart side, `buildApprovedItems` (`src/api/lib/cart_selection.ts` line 28) explicitly gates every row on `row.is_approved && row.selected_product_id`, skipping anything else — so nothing reaches `cart_runner.ts` without one of those two approval states. No gap found.
- [x] A partial cart-add result always lists every unadded item with a reason. `cart_runner.ts` pushes every non-`added` outcome as `status:"needs_attention"` with a `reason` string sourced from `reasonFromAddToCartFailure` (real Kroger API rejection text) or a synthesized message (e.g. "all N candidates rejected", network-error text) — never an empty reason on that path; covered extensively by `src/kroger/cart_runner.test.ts` (product-not-found, fallback-used, all-candidates-rejected, network-error cases all assert on `result.reason`). `CartResult.tsx`'s `NeedsAttentionItem` renders `item.reason` for every item in the "Needs attention" section. No gap found.
- [x] Deleting a recipe removes it from the list and from the backend within one request/response cycle. Verified live against production, not just code reading: created a test job, then `DELETE /api/recipes/<id>` returned `204`, and an immediate follow-up `GET /api/recipes/<id>` (no delay) returned `404` — confirming the backend delete (`src/api/routes/recipes.ts`'s `DELETE /:id`, which deletes the `recipes` and `jobs` rows synchronously in the same handler) completes fully within that one request. Frontend (`RecipeCard.tsx`'s `handleDelete`) calls `onDeleted?.(item.id)` immediately after `await apiDelete(...)` resolves — i.e. in the same response cycle — which synchronously filters the row out of local list state (`RecipesList.tsx`'s `handleDeleted`); the subsequent `fetchRecipes()` call in that same handler is a background re-sync, not a prerequisite for the row disappearing. (Test job deleted as part of this same check, so no residue left in production.)
- [ ] All primary flows are operable via VoiceOver and at the largest Dynamic Type setting. **Documented, accepted exception — not a fresh gap.** Accessibility work was explicitly deprioritized on 2026-07-20 per `files/phases.md`'s Phase 5 section: this is a hobby project with no ADA-compliance obligation, and the decision was to revisit only if real demand emerges. Leaving this unchecked reflects that accepted tradeoff, not new evidence of breakage — some VoiceOver-friendly groundwork already exists incidentally (e.g. `aria-label`/`role="alert"`/`visually-hidden` labels scattered through `IngredientCard.tsx`, `RecipeCard.tsx`), but no deliberate audit or real-device VoiceOver pass has been done, consistent with the documented deprioritization.
