...
- [x] A valid single-video public TikTok recipe URL produces a structured recipe with confidence and evidence for every populated field within the 5-minute hard timeout. **PASS (2026-07-20 fix).** `src/worker/state_machine.ts`'s `runJob` now races the `extract()` call against a `setTimeout(config.extraction.jobTimeoutMs)` via `Promise.race` — on timeout it constructs a new `extraction_timeout` `ExtractionError` (added to the `FailureClass` taxonomy, `src/pipeline/extract/failures.ts`) and persists it through the exact same upsert mechanism every other failure class uses (`persistFailure`, exported from `src/pipeline/extract/index.ts`), so `recipes.failure_class` ends up populated identically regardless of which side detected the failure. The abandoned `extract()` call (no cooperative cancellation exists for the underlying yt-dlp/Whisper/Claude calls) has its eventual settlement swallowed so it can never surface as an unhandled rejection. Proven by `src/worker/state_machine.test.ts` — "classifies a hung extract() as extraction_timeout, terminates the job, and persists the failure row" mocks a never-resolving `extract()`, lowers `config.extraction.jobTimeoutMs` to 20ms, and asserts both the `jobs` row reaches `Failed` and the `recipes` row's `failure_class` is `extraction_timeout`.
- [x] No field is populated without at least one evidence reference; unsupported fields are `null` with a stated reason. **PASS (2026-07-20 fix).** `QuantitySchema` (`src/pipeline/schema.ts`) now carries the same evidence discipline as every other field: `value`/`unit` are gated by a `superRefine` requiring `evidence.length >= 1` when `value` is non-null, or a stated `null_reason` when `value` is null — reusing `EvidenceRefSchema`/`ConfidenceBandSchema`/`ConflictRecordSchema` unchanged. `raw_text` sits outside the gate and stays unconditionally required (never fabricated, never dropped, regardless of `value`'s state). This isn't a drop-in `evidencedField()` wrap — quantity has multiple related sub-fields, not one scalar — so the shape keeps `quantity.value`/`quantity.unit`/`quantity.raw_text` at the top level (no downstream restructuring needed for the matcher, ingredient persistence, or DTOs) while adding `evidence`/`null_reason`/`confidence`/`conflict` alongside. The reconcile system prompt (`src/pipeline/extract/reconcile.ts`, rules 1/2/8/9) now explicitly instructs Claude to emit `quantity.evidence`/`quantity.null_reason`/`quantity.confidence` the same way it already does for `canonical_name_en`. Proven by `src/pipeline/schema.test.ts` — "rejects a quantity with a non-null value and zero evidence refs (the gap this schema closes)" and "rejects a null quantity value with no null_reason" — the direct inverse of the previously-passing gap-proving test.
- [x] A non-recipe video returns the distinct `recipe.not_a_recipe` result, not a garbled recipe or a generic error. **PASS (2026-07-20 fix).** `RecipeSchema.result_type` still correctly distinguishes `"recipe"` vs `"not_a_recipe"` (`src/pipeline/schema.ts:115`), and now `src/worker/state_machine.ts`'s `not_a_recipe` branch calls `setRecipeFailureClass(recipeId, "not_a_recipe", ...)` (`src/platform/jobs.ts`, the same UPDATE-only mechanism this session's Kroger connect/reconnect fix already added) before `finishJob`, giving the recipe row a `failure_class` distinct from every genuine technical failure. `web/src/lib/failureCards.ts` adds a `not_a_recipe` entry ("This doesn't look like a recipe video." / "Back to recipes", via `recoveryHref: () => "/"` rather than the default reprocess button, since retrying would only reach the same conclusion) so `FailureCard.tsx` renders a friendly, distinct card instead of the generic fallback. Proven by `src/worker/state_machine.test.ts` — "routes result_type=not_a_recipe through a distinct failure_class, not the generic failure path" asserts the `recipes` row keeps `status: "extracted"` (untouched) while gaining `failure_class: "not_a_recipe"`, and the `jobs` row still reaches `Failed` with `last_error: "not_a_recipe"`.
- [x] A private/deleted/region-locked video fails immediately with a specific, non-retryable reason. **PASS.** `classifyDownloadFailure()` (`src/pipeline/extract/failures.ts:60-64`) matches yt-dlp stderr against `PERMANENT_DOWNLOAD_PATTERNS` (private/deleted/removed/region/age-restricted/login-required, `failures.ts:43-54`) and maps to `download_failed_permanent`, which `isRetryableDownloadError()` (`download.ts:32-38`) treats as non-retryable. Directly proven by `src/pipeline/extract/download.test.ts:91-101` — "maps a permanent (private/deleted) failure to a terminal ExtractionError with NO retry" asserts `spawnMock` was called exactly once (no retry attempt), and by `src/pipeline/extract/failures.test.ts:9-19`'s parametrized cases for all the private/deleted/region/age-restricted stderr strings. Reason text is specific per `userFacingReasonFor("download_failed_permanent")` (`failures.ts:69-70`).
- [x] A conflicting quantity between narration and on-screen text is retained as a flagged warning, never silently resolved without a trace. **PASS (2026-07-20 fix).** `QuantitySchema` (`src/pipeline/schema.ts`) now has a `conflict?: ConflictRecordSchema` field, identical in shape to the one already used on `canonical_name_en`/`title`/`steps[]` entries: `{ resolved_source, alternatives: [{ source_type, value }] }`. The reconcile system prompt's rule 9 (`src/pipeline/extract/reconcile.ts`) explicitly walks through the PRD's own canonical example — narration "1 cup sugar" vs on-screen "3/4 cup" — instructing Claude to resolve `quantity.value` per the existing on-screen > caption > narration priority (rule 3) while recording the discarded narration reading on `quantity.conflict.alternatives` rather than dropping it. Proven by `src/pipeline/schema.test.ts` ("retains a conflicting quantity between narration and on-screen text as a flagged warning, never silently resolved (PRD C2 §26)") and `src/pipeline/extract/reconcile.test.ts` ("passes confidence bands and a conflict record through validation," extended to assert `quantity.conflict.resolved_source`/`alternatives` on the sugar-quantity fixture).
- [x] No temp media file for a completed job exists on disk more than a few seconds after the job reaches a terminal state, and none exist past the 24-hour TTL backstop under any circumstance. **PASS, with margin.** `cleanupTempDir(jobId)` (`src/platform/db.ts:16-19`, synchronous `fs.rmSync`) runs in `extract()`'s `finally` block (`src/pipeline/extract/index.ts:175-179`) on every terminal path — success or thrown `ExtractionError` — covering the "few seconds after terminal state" half. The TTL backstop is real and tighter than required: `config.tempMedia.ttlHours = 1` (`src/platform/config.ts:191`, lowered from 6 on 2026-07-20 per the comment at `config.ts:182-189` after a real incident — a burst of OOM-crash SIGKILLs bypassed normal cleanup and filled the 500MB Railway volume within about an hour, commit `c456760`). The sweep itself is real and tested: `sweepTempMedia()` (`src/worker/sweeps.ts:18-32`) compares dir mtimes against `ttlHours`, is wired into the worker's periodic loop (`src/worker/index.ts:73-83`), and is covered by `src/worker/sweeps.test.ts:70-101`. 1h ≤ 24h satisfies the criterion with margin.
