...
- **`[P0/P1]` local:** Python or Node runtime (decision A4-1 in Spec 4 — gates this spec's language), `yt-dlp`, `ffmpeg` (Homebrew locally), OCR engine (A2-1), ASR API key (A2-2), `ANTHROPIC_API_KEY` in `.env`. Per-job temp dir under the local data dir, deleted at terminal state.
- **`[P4]` Docker:** all of the above layered into the single Spec 4 image; API keys via Railway env vars; temp dir on the worker volume with the 24h TTL sweep as backstop.

## 6. Open Action Items

- [ ] **A2-1 — OCR engine selection.** Recommendation: start with a cloud OCR API (e.g., Google Cloud Vision) for P1 — meaningfully better than Tesseract on stylized TikTok overlay text, trivially swappable later since OCR is one stage function; Tesseract as a zero-cost fallback if the cloud dependency feels heavy. Needs confirmation before P1.
- [ ] **A2-2 — ASR provider selection.** Recommendation: OpenAI Whisper API or Deepgram — both give segment timestamps and auto language detection; local Whisper is viable but adds model weight to the Docker image. Needs confirmation before P1.
...
- **B2-3 — ASR/OCR vendor accounts + keys** (depends on A2-1/A2-2 decisions). Trivial but gates P1 start.
- **B2-4 — Anthropic API key with billing.** Gates P1.

## 8. Considerations

- **Setup:** yt-dlp pin + the download-failure-by-category metric double as the TikTok-breakage early-warning system — build the failure-class logging even in P1, since it costs little and the signal matters immediately. The Docker image (P4) will be large (Chromium + ffmpeg + OCR deps); set up build caching from the first Dockerfile.
