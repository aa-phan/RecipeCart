...
- [x] No item is ever added to the Kroger cart without a corresponding explicit approval event. Verified: `POST /recipes/:id/cart:approve` (`src/api/routes/cart.ts` line ~16) requires an `Idempotency-Key` header (400 `badRequest` otherwise) and sources its item list exclusively from `buildApprovedItems` (`src/api/lib/cart_selection.ts` line 28), which skips any `product_matches` row where `is_approved` is false or `selected_product_id` is null. `runCartApproval` (`src/kroger/cart_runner.ts` line 506) never calls `addToCart` for anything outside that list. The only other caller is `recipecart approve <recipe-id>` in `src/cli.ts` (line ~225), itself a user-typed explicit command. `src/api/routes/cart.test.ts` + `src/kroger/cart_runner.test.ts` (81 tests total across the cart/matcher suite, all passing as of this check, run 2026-07-20) cover the approved-only path.
- [x] No code path exists that can reach a checkout/payment endpoint (verified by the fact that the underlying API has none). Verified by direct grep of `src/kroger/client.ts`: the only Kroger HTTP calls in the entire codebase are `GET /products` (`searchProducts`), `GET /locations` (`searchLocations`), and `PUT /cart/add` (`addToCart`, line 86). No `checkout`, `payment`, or `order` path string appears anywhere in `client.ts` or `types.ts` — there is nothing to call because Kroger's Public API doesn't expose it.
- [x] An expired or revoked OAuth token always pauses the job and prompts the user to reconnect; it is never silently retried against an invalid token. Verified end-to-end: proactive refresh in `ensureValidUserToken` (`src/kroger/cart_runner.ts` line 106) refreshes near/past-expiry tokens before a run starts, and reactive 401 handling in `addItemWithRetry`/`processItems` (lines 260-264, 393-401) stops the loop immediately on a 401 rather than treating it as a per-item failure or retrying it. Both paths terminate the job at `requires_user_intervention` with a `failureClass` of `kroger_not_connected` or `kroger_token_expired` (lines 55, 469, 498, 559, 584), persisted onto the `recipes` row via `setRecipeFailureClass` (`src/api/routes/cart.ts` line ~40, `src/platform/jobs.ts` line 263) and surfaced by `web/src/lib/failureCards.ts` (both classes mapped to a message + `/connect-kroger?resumeRecipeId=...` link) with a working resume round-trip in `web/src/App.tsx` (lines 8-35, reads `KROGER_RESUME_STORAGE_KEY` and navigates back to the recipe on reconnect). Confirmed this session via commit `83323bc` ("Fix Kroger connect/reconnect flow end-to-end") and by reading the current code directly rather than trusting the commit message — the 401 branch never falls through to a retry, and `TRANSIENT_RETRY_ATTEMPTS` (line 93) is explicitly scoped to network errors only (`addItemWithRetry`'s catch block, lines 271-283), never to a clean 401 response. `src/kroger/cart_runner.test.ts` exercises both the pre-flight-missing-token and mid-run-401 cases.
- [x] A weight-sold item is clearly labeled as an estimate in both the match and the cart result. Fixed this session. Match side: `web/src/screens/Review/MatchPicker.tsx` now renders `candidate.reason` as a small muted/italic note (`.match-picker__candidate-note`, `web/src/screens/Review/Review.css`) — both in the collapsed pre-selected summary and in each row of the expanded candidate list. Cart-result side: `reason` needed new threading. `ProductCandidate.reason` (already computed by the matcher, `src/matcher/index.ts`) was not carried from `buildApprovedItems` (`src/api/lib/cart_selection.ts`) onto `ApprovedCartItem`/its `fallbacks`, so `AddedItem` had nothing to read even though `CartItemResult.reason` already existed as a field (previously used only for `needs_attention` failure text and a fallback-candidate-used note). Threaded `reason` through the same display-only path already used for `productName`/`imageUrl`/`price`: `buildApprovedItems` now sets `reason: selected.reason` (and per-fallback `reason: c.reason`); `addItemWithFallback`/`processItems` (`src/kroger/cart_runner.ts`) carry it onto the `added` result, combining it with the existing "fallback candidate used" note (rather than one overwriting the other) when both apply. `web/src/screens/CartResult/CartResult.tsx`'s `AddedItem` now renders `item.reason` via the existing `.cart-result__reason` style (previously only used by `NeedsAttentionItem`). Verified via `npx tsc --noEmit -p .`, `cd web && npm run build`, and the existing `src/kroger/cart_runner.test.ts` + `src/api/routes/cart.test.ts` suites (69 tests, all passing) — the fallback-note assertions use `stringContaining`, unaffected by the combined-reason change.
- [x] A partial cart-add result lists every unadded item with a specific reason. Verified in `processItems` (`src/kroger/cart_runner.ts` lines 366-418): every item whose `addItemWithFallback` outcome isn't `added` or `auth_failure` gets a `needs_attention` result with a non-empty `reason` (`reasonFromAddToCartFailure`, lines 128-143, always returns a string, falling back to `` Kroger API error ${status} `` when nothing more specific is available). The web `CartResult.tsx` renders `item.reason` for every `NeedsAttentionItem` (line 190). Caveat: this guarantee is scoped to the `partially_completed` terminal state specifically — when a run instead hits `requires_user_intervention` (mid-run 401), items after the failure point are left unlisted (neither `added` nor `needs_attention`) rather than itemized, because their state is genuinely unattempted; `resumeCartRun` (lines 427-500) picks them back up on reconnect rather than reporting them as a specific-reason failure. That's a different, deliberate code path, not a gap in this criterion as scoped to partial-completion.
- [x] A material substitution is never pre-selected without being visibly flagged as such. Verified two ways. (1) Persistence gate: `persistMatch` (`src/matcher/index.ts` lines 1006-1051) only sets `selected_product_id`/`is_approved` when `!match.requiresApproval` (line 1022) — a match with `requiresApproval === true` is never pre-selected, full stop, and the web `MatchPicker.tsx` always renders `requiresApproval === true` matches expanded with no candidate pre-checked (lines 33, 98-99, 111). (2) The new dietary-conflict mechanism: `DIETARY_CONFLICTS` (`src/matcher/index.ts` lines 137-142) and `applyPreferenceAdjustments` (lines 211-255) snapshot the would-be winner before vs. after the dietary penalty; if the penalty changes the winner, `dietaryOverrideTag` is returned and `matchIngredient` (lines 739-743) forces `requiresApproval = true` with an explicit "Deprioritized for your ... preference" note — this composes correctly with the pre-existing broadened-search and close-score-ambiguity flags (same `requiresApproval` flag, same `persistMatch` gate downstream), it doesn't bypass them. Covered by `src/matcher/index.test.ts` (dietary-conflict-forces-approval cases) and the full 81-test cart/matcher suite passing as of this check (commit `34e03a7`, "Wire Preferences into product ranking").
- [x] Daily API call volume stays observably within Kroger's documented rate limits. Fixed by adding a structured log line to `recordCall` (`src/kroger/rate_limit.ts`): every time a live Kroger call is counted (all three call sites in `src/kroger/client.ts` — `searchProducts`, `searchLocations`, `addToCart` — go through `recordCall`), it now emits `logger.info("kroger: rate limit usage", { endpoint, currentCount, dailyLimit, remaining })` using the real post-increment count (via the upsert's `RETURNING count`) and the same `ceilingFor(endpoint)` used by `assertUnderLimit`. Logs on every call rather than only at a threshold — deliberate: at single-user volume this is at most dozens of lines per matching run, not a scale where per-call logging is noisy, and a running count is simpler to grep/alert on than a sampled one. No new route or CLI command was added (an authenticated endpoint would be heavier than this gap calls for; an unauthenticated one would leak usage-pattern info for no benefit) — this satisfies §20's "daily API call volume against the documented rate-limit ceilings" early-warning signal via existing structured logs, matching how every other early-warning signal in this codebase is surfaced. Covered by `src/kroger/rate_limit.test.ts` ("recordCall usage logging" — asserts the exact fields and values logged, including per-endpoint ceilings).
