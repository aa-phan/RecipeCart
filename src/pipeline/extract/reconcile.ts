  ...
  "ingredients": [
    {
      ...
      "quantity": {
        "value": number | null,
        "unit": string | null,
        "raw_text": string,                       // ALWAYS populated, regardless of value/null — see rule 2
        "evidence"?: [{ "source_type": "asr"|"ocr"|"caption", "timestamp"?: number, "frame_ref"?: string, "snippet"?: string }],  // required when value is non-null
        "null_reason"?: string,                    // required when value is null — see rule 2
        "confidence"?: "high" | "medium" | "low",  // on every non-null quantity — see rule 8
        "conflict"?: { "resolved_source": "asr"|"ocr"|"caption", "alternatives": [{ "source_type": "asr"|"ocr"|"caption", "value": string }] }  // only when sources disagreed on the quantity — see rule 9
      },
...
1. EVERY field that is an { value, evidence, null_reason } object ("evidenced field") MUST follow exactly one of these two forms:
   ...
   Never provide a non-null value without evidence. Never leave a null value without a null_reason. This rule applies to "quantity" exactly the same way it applies to "canonical_name_en", "title", and "steps" entries — quantity.value/quantity.evidence/quantity.null_reason follow the identical two forms above (quantity.unit travels with quantity.value: when value is null because the amount was vague, leave unit null too, evidenced by the same null_reason). quantity.raw_text is the ONE exception: it is always required and always populated regardless of whether value ended up non-null or null — never fabricate raw_text, but never omit it either.
2. NEVER infer or guess a quantity, ingredient, or amount that isn't actually stated in the evidence. If a quantity is vague ("a glug of olive oil", "some salt"), set quantity.value to null and quantity.unit to null, and record a quantity.null_reason (e.g. "amount given only as 'a glug', not a measurable quantity"), but KEEP quantity.raw_text as the literal text ("a glug", "some"). Do not convert vague amounts into fabricated numbers. When a quantity IS stated as a real number, evidence it in quantity.evidence exactly like any other evidenced field (rule 1).
...
8. For EVERY evidenced field ({value, evidence, null_reason} object) that has a non-null value — including "quantity" — add a "confidence" band: "high" when the value is stated directly and unambiguously in the evidence (a clear on-screen quantity, an explicit spoken ingredient), "medium" when it's evidenced but somewhat indirect or paraphrased, "low" when it's the weakest reading you'd still stand behind. Confidence is about how well the EVIDENCE supports the value — it is NOT a license to include unevidenced values at "low"; an unevidenced value is still null + null_reason (rule 1), never a low-confidence guess. Omit "confidence" on null fields.
9. When two evidence sources DISAGREE on the same field's value (e.g. OCR shows "2 cups flour" but narration says "about a cup"), resolve it by the rule-3 priority (ocr > caption > asr), put the winning value in "value" with its evidence, AND record the discarded reading(s) in "conflict": { resolved_source, alternatives: [{source_type, value}] }. Only include "conflict" when there was a genuine cross-source disagreement — omit it otherwise. This applies to "quantity" just as much as any other field: the PRD's canonical example is narration saying "1 cup sugar" while on-screen text says "3/4 cup" — resolve quantity.value to 0.75 with unit "cup" and evidence pointing at the OCR (rule 3 priority), and set quantity.conflict to { resolved_source: "ocr", alternatives: [{ source_type: "asr", value: "1 cup" }] } so the discarded narration reading is retained, never silently dropped. A quantity conflict can also be recorded when only the numeric value differs but the unit doesn't (or vice versa) — use judgment: if either sub-part of the amount is genuinely contested between sources, flag it.`;
